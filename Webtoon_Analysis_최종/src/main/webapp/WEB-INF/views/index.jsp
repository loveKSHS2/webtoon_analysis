...
<!--
                ...
                <div class="card-body">
                  ...
                  anychart.onDocumentReady(function () {
                    ...
                    <c:forEach items="${keywordList}" var="test">
                    	<c:if test="${test.sep == '작가' && test.episode == ''&& test.word == '스토리' || test.word == '그림' || test.word == '생각' || test.word == '진짜' || test.word == '작품'}">
              ...
              <div class="card shadow mb-4">
                ...
                <div class="card-body">
                  ...
                  anychart.onDocumentReady(function () {
                    ...
                    var data = [ 
                    <c:forEach items="${keywordList}" var="test">
                    	<c:if test="${test.sep == '작품' && test.word == '내용' || test.word == '사람' || test.word == '생각' || test.word == '말' || test.word == '여주'}">
							{"x": "${test.related_word1}", "value": ${test.frequency}, category: "${test.word}"},
 	                  		{"x": "${test.related_word2}", "value": ${test.frequency},category: "${test.word}"},
 	                 		{"x": "${test.related_word3}", "value": ${test.frequency}, category: "${test.word}"},
 	                		{"x": "${test.related_word4}", "value": ${test.frequency}, category: "${test.word}"},
          ...
          <div class="row">
            <div class="col-xl-5 col-lg-9">
              <div class="card shadow mb-4">
                <!-- Card Header - Dropdown -->
                <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                  <h6 class="nanum m-0 font-weight-bold text-primary">유로독자 키워드</h6>
                  <div class="dropdown no-arrow">
                    <a class="dropdown-toggle" href="#" role="button" id="dropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                      <i class="fas fa-ellipsis-v fa-sm fa-fw text-gray-400"></i>
                    </a>
                ...
                <div class="card-body">
                  ...
                  anychart.onDocumentReady(function () {
                    ...
                    var data = [
                    <c:forEach items="${keywordList}" var="test">
                    	<c:if test="${test.sep == '유료독자' && test.word == '사람' || test.word == '돈' || test.word == '보라' || test.word == '개' || test.word == '베댓'}">
							{"x": "${test.related_word1}", "value": ${test.frequency}, category: "${test.word}"},
 	                  		{"x": "${test.related_word2}", "value": ${test.frequency}, category: "${test.word}"},
 	                 		{"x": "${test.related_word3}", "value": ${test.frequency}, category: "${test.word}"},
 	                		{"x": "${test.related_word4}", "value": ${test.frequency}, category: "${test.word}"},
              ...
              <div class="card shadow mb-4">
                <!-- Card Header - Dropdown -->
                <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                  <h6 class="nanum m-0 font-weight-bold text-primary">구독취소 키워드</h6>
                  <div class="dropdown no-arrow">
                    <a class="dropdown-toggle" href="#" role="button" id="dropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                      <i class="fas fa-ellipsis-v fa-sm fa-fw text-gray-400"></i>
                    </a>
                ...
                <div class="card-body">
                  ...
                  anychart.onDocumentReady(function () {
                    ...
                    var data = [
	               	<c:forEach items="${keywordList}" var="test">
                    	<c:if test="${test.sep == '구독취소' && test.word == '친구' || test.word == '웹툰' || test.word == '보라' || test.word == '말' || test.word == '작가'}">
							{"x": "${test.related_word1}", "value": ${test.frequency}, category: "${test.word}"},
 	                  		{"x": "${test.related_word2}", "value": ${test.frequency},category: "${test.word}"},
 	                 		{"x": "${test.related_word3}", "value": ${test.frequency}, category: "${test.word}"},
 	                		{"x": "${test.related_word4}", "value": ${test.frequency}, category: "${test.word}"},
  ...
  <script type="text/babel">
    ...
    function generateData(count, data) {
      ...
      var x = (2).toString();         //x축 라벨
      var y = 44 + 20                       //데이터 넣는 부분
      series.push({                      //한줄에 넣을 데이터 series에 push
          x: x,
          y: y
        });

        var x = (3).toString();         //x축 라벨
      var y = 43                       //데이터 넣는 부분
      series.push({                      //한줄에 넣을 데이터 series에 push
          x: x,
          y: y
        ...
        var x = (4).toString();         //x축 라벨
      var y = 48                       //데이터 넣는 부분
      series.push({                      //한줄에 넣을 데이터 series에 push
          x: x,
          y: y
        });
        var x = (5).toString();         //x축 라벨
      var y = 46                       //데이터 넣는 부분
      series.push({                      //한줄에 넣을 데이터 series에 push
          x: x,
          y: y
        });
        var x = (6).toString();         //x축 라벨
      var y = 48 + 20                      //데이터 넣는 부분
      series.push({                      //한줄에 넣을 데이터 series에 push
          x: x,
          y: y
        });

       var x = (7).toString();         //x축 라벨
      var y = 47                       //데이터 넣는 부분
      series.push({                      //한줄에 넣을 데이터 series에 push
          x: x,
          y: y
...
function generateData2(count, data) {
      var i = 0;
      var series = [];
    ...
    function generateData3(count, data) {
      var i = 0;
      var series = [];
    ...
    function generateData4(count, data) {
      var i = 0;
      var series = [];
...
function generateData5(count, data) {
      var i = 0;
      var series = [];
    ...
    class HeatmapChart extends React.Component {
      ...
      series: [
            ...
            {
              name: '20화',
              data: generateData2(10, 31*2)       //넣을 데이터가 차이가 많이 안나기 떄문에 재량 것 곱하기 해서 차이 극대화해서 보이기
            },
            {
              name: '30화',
              data: generateData3(10, 30)
            },
            {
              name: '40화',
              data: generateData4(10, 30)
            },
            {
              name: '50화',
              data: generateData5(10,30)
            }
          ],     
